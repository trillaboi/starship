...
const Markets: NextPage = () => {
    ...
    const handleClick = (id: number) => {
        // Get the selected market based on the id
        const selectedMarket = markets.find((market) => market.id === id);
    
        if (selectedMarket) {
          // Open a new webpage with the details of the selected market
          window.open(`/market-details/${selectedMarket.id}`, '_blank');
        }
      };
    ...
    return(
        <div className={styles.container} >
            ...
            <div className={styles.marketContainer}> 
                {markets.map((market) => (
                    <div className={styles.marketCard} key={market.id} onClick={() => {handleClick(market.id)}}>
                        <h2>{market.title}</h2>
